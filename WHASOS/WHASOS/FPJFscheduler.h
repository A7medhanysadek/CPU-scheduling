...
struct fpjf
{
	...
	// readyQueue is a priority queue that sorts the processes based on their priority, then arrival time, then process ID,using the comparator in process.h
	...
	{
		string readyQueueString = "";
		priority_queue<process, vector<process>, comparePriority> temp = readyQueue;
		while (!temp.empty())
		{
